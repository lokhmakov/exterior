...
class EditorSection extends Component {
  ...
  renderGroupForegroundAdd = ({ doc, productMutate }) => {
    return (
      <Mutation
        name='groupCreate'
        entity='group'
  ...
  renderGroupQuery = (projectDoc, productMutate) => ({ data, order }) => {
    const pathListForeground = order.reduce((result, id) => {
      if (data[id].type === `foreground`) {
        result.push(data[id].path)
      }
      return result
    }, [])
      ...
      return (
        <React.Fragment>
          <Group2d
            ...
            scale={ projectDoc.scale }
            pathListForeground={ pathListForeground }
          />
          {
      ...
      >
        {({ mutate }) => (
          <React.Fragment>
            ...
            { mode === modeMap.GROUP_FOREGROUND_ADD && this.renderGroupForegroundAdd({ doc, productMutate: mutate }) }
